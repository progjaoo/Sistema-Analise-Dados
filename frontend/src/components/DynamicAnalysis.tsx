...
function DailyHeatmap({ data }: { data: AnalysisResponse }) {
  const points = useMemo(() => audiencePoints(data).filter((point) => isMaravilha(point.station) || !point.station || point.station === "Geral"), [data]);
  ...
  const max = Math.max(...points.map((point) => point.opm), 0);
  const map = useMemo(() => new Map(points.map((point) => [`${point.dayKey}|${point.time}`, point])), [points]);
  if (!points.length || !times.length) return <Empty title="Maravilha FM Dia a Dia" text="Não foi possível identificar dia, horário e OPM." />;
  return <Panel title="Maravilha FM Dia a Dia" subtitle="Heatmap de audiência por dia e horário."><div className="heatmap-wrap"><div className="heatmap-lite" style={{ gridTemplateColumns: `110px repeat(${times.length}, minmax(46px, 1fr))` }}><div className="heatmap-head">Dia</div>{times.map((time) => <div key={String(time)} className="heatmap-head">{time}</div>)}{days.map((day) => <div className="contents" key={day.key}><div className="heatmap-label">{day.label}</div>{times.map((time) => { const point = map.get(`${day.key}|${time}`); return <div key={`${day.key}-${time}`} className="heatmap-cell" style={{ background: point ? heatColor(point.opm, max) : "#f1f5f9", color: point && point.opm / (max || 1) > .45 ? "#fff" : "#334155" }} title={`${day.label}\nHorário: ${time}\nOPM: ${format(point?.opm ?? null)}`}>{point ? format(point.opm) : "—"}</div>; })}</div>)}</div></div></Panel>;
}
