...
export function createApp({ repository = createRepository(), env = process.env }: { repository?: Repository; env?: NodeJS.ProcessEnv } = {}) {
  ...
  const origins = (env.FRONTEND_ORIGIN || "http://localhost:5173").split(",").map((origin) => origin.trim().replace(/\/$/, "")).filter(Boolean);

  app.disable("x-powered-by"); app.set("trust proxy", 1); app.use(pinoHttp({ logger }));
  app.use(cors({ origin(origin, callback) { if (!origin || origins.includes(origin.replace(/\/$/, ""))) return callback(null, true); return callback(new Error("Origem não autorizada pelo CORS.")); }, credentials: true }));
