...
export function userRoutes(controller: ReturnType<typeof createUserController>, auth: ReturnType<typeof createAuthMiddleware>) {
  const router = Router(); router.use(auth.authenticate, auth.requireAdmin); router.get("/", controller.list); router.post("/", controller.create); router.patch("/:id/active", controller.setActive); return router;
}
